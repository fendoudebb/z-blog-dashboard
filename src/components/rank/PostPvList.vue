<template>
  <div style="margin: 20px;padding:20px;background-color: white;">
    <Table border stripe :highlight-row="true" :data="postPvList" :columns="postPvListColumns" :loading="postPvListTableLoading"></Table>
  ...
  export default {
    ...
    data() {
      return {
        ...
        postPvListTableLoading: false,
        pageSize: this.getPvListSize(),
        totalCount: 1,
        currentPage: 1,
        postPvList: [],
        postPvListColumns: [
    ...
    methods: {
      ...
      changePage(index) {
        this.setPvListPage(index);
        this.getPostPvList();
      },

      getPostPvList() {
        this.postPvListTableLoading = true;
        this.handlePostPvList().then(value => {
          this.totalCount = value.data.totalCount;
          this.postPvList = value.data.post;
          this.postPvListTableLoading = false;
        }).catch(reason => {
          this.postPvListTableLoading = false;
        })
      }
